...
constexpr uint8_t  TURBINE_TOTAL       = 6;
constexpr uint8_t  BLADES_PER_TURBINE  = 13;
...
struct DynamicPositioningConfig {
    bool  enabled                = true;
    float kp                     = 1.2f;
    float disturbance_horizon    = 4;
    float feedforward_gain       = 0.9f;
    float max_thrust_m           = 0.15f;
    float flow_base_m            = 0.06f;
};
...
public:
    explicit DisturbanceObserver(size_t horizon = 4) : horizon_(horizon) {}

    void record(const std::array<float, 3>& disturbance) {
        std::lock_guard<std::mutex> lk(mtx_);
        history_.push_back(disturbance);
        if (history_.size() > horizon_) history_.pop_front();
    }

    std::array<float, 3> estimate() const {
        std::lock_guard<std::mutex> lk(mtx_);
        std::array<float, 3> sum{};
        if (history_.empty()) return sum;
        for (const auto& d : history_) for (int i = 0; i < 3; ++i) sum[i] += d[i];
...
public:
    DynamicPositioner() = default;

    void setConfig(const DynamicPositioningConfig& cfg) { cfg_ = cfg; }
    const DynamicPositioningConfig& config() const { return cfg_; }

    RobotPose correct(const RobotPose& measured,
                      const RobotPose& reference,
                      const std::array<float, 3>& measured_disturbance);

    void reset() { observer_.clear(); }

private:
    DynamicPositioningConfig cfg_;
    DisturbanceObserver observer_{4};
};
...
private:
    ...
    std::vector<int32_t> near(const Point3D& p, float radius);
...
public:
    ...
    DynamicPositioner& positioner() { return positioner_; }
    const DynamicPositioner& positioner() const { return positioner_; }

    void setDynamicPositioningEnabled(bool enabled) {
        DynamicPositioningConfig cfg = positioner_.config();
        cfg.enabled = enabled;
        positioner_.setConfig(cfg);
    }
